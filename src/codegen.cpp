    ...
    if (f->fptr == &jl_apply_generic) {
        ...
        *ret = mark_julia_type(lty, true, jl_datatype_type);
        ...
        if (jl_subtype(ty, (jl_value_t*)jl_type_type, 0)) {
            ...
            *ret = emit_expr(args[1], ctx);
#ifdef LLVM37
            builder.CreateCall(prepare_call(typeassert), {get_gcrooted(*ret, ctx), boxed(emit_expr(args[2], ctx),ctx)});
#else
            builder.CreateCall2(prepare_call(typeassert), get_gcrooted(*ret, ctx), boxed(emit_expr(args[2], ctx),ctx));
#endif
            JL_GC_POP();
            return true;
        }
